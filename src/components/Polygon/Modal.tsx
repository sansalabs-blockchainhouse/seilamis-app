...
export default function Modal({
  ...
  const { data: collections } = useQuery({
    queryKey: ["collections-polygon-verified"],
    queryFn: (): Promise<ICollection[]> =>
      api.get(`raffle-polygon-verified`).then((response) => response.data),
    ...
    if (currentCollection?.address) {
      try {
        ...
        const { data } = await api.get(
          `raffle-polygon-verified/${currentCollection.address}/${address}`
        );
  ...
  return (
    ...
    >
      <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative bg-secondary rounded-lg shadow border border-white">
